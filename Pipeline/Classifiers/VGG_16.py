...
class VGG_16(Classifier_base):
    ...
    def build(self):
        ...
        VGG = VGG16(include_top=False, weights='imagenet', input_shape=(self.size[0], self.size[1], 3))
        ...
        model = self.model = Model(VGG.input, output)
    ...
    def get_datagen(self, aug=False):
        if aug:
            return ImageDataGenerator(rotation_range=10, width_shift_range=0.05, zoom_range=0.05,
                                      ...
                                      horizontal_flip=True, preprocessing_function=VGG_preprocess)
        return ImageDataGenerator(preprocessing_function=VGG_preprocess)
